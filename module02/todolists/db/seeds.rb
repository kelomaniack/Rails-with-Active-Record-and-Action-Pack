...
profiles.each do |profile|
  profile.create_user( username: profile.last_name, password_digest: profile.last_name)
...
todo_lists.each do |list|
  5.times do |i|
    list.todo_items.create!(due_date: due_date, title: "Title" + "#{i}", description: "Description" + "#{i}")
  end
end
